...
local Library = {
    ...
    OutlineColor = Color3.fromRGB(78, 66, 108);
...
function Library:StopWindowDrag()
    if WindowDrag.MoveConn then
        WindowDrag.MoveConn:Disconnect();
        WindowDrag.MoveConn = nil;
    end;
    if WindowDrag.EndConn then
        WindowDrag.EndConn:Disconnect();
        WindowDrag.EndConn = nil;
    end;
...
function Library:MakeDraggable(Target, HandleOrCutoff)
    local Handle = Target;
    local Cutoff = nil;
    if typeof(HandleOrCutoff) == 'Instance' then
        Handle = HandleOrCutoff;
    elseif type(HandleOrCutoff) == 'number' then
        Cutoff = HandleOrCutoff;
    end;
...
function Library:GetViewportSize()
    ...
    local screenResolutionOk, res = pcall(function()
        return GuiService:GetScreenResolution();
    end);
    if screenResolutionOk and typeof(res) == 'Vector2' and res.X > 0 and res.Y > 0 then
        return res;
    end;
...
function Library:IsMobile()
    if self._IsMobileCached ~= nil then
        return self._IsMobileCached;
    end;
    ...
    if preferredOk and preferred == Enum.PreferredInput.Touch then
        self._IsMobileCached = true;
        return true;
    end;

    local smallScreenOk, isSmall = pcall(function()
        return GuiService:IsSmallScreen();
    end);
    if smallScreenOk and isSmall == true then
        self._IsMobileCached = true;
        return true;
    end;

    if InputService.TouchEnabled == true then
        if InputService.KeyboardEnabled == false or InputService.MouseEnabled == false then
            self._IsMobileCached = true;
            return true;
        end;
        if InputService.AccelerometerEnabled == true or InputService.GyroscopeEnabled == true then
            self._IsMobileCached = true;
            return true;
        end;
        local vp = self:GetViewportSize();
        if vp.Y < 560 or vp.X < 860 then
            self._IsMobileCached = true;
            return true;
        end;
    end;

    self._IsMobileCached = false;
    return false;
end;

function Library:GetResponsiveWindowSize()
    local viewport = self:GetViewportSize();
    local vx = viewport.X;
    local vy = viewport.Y;

    if not self:IsMobile() then
        local w = math.min(840, math.floor(vx * 0.92));
        local h = math.min(600, math.floor(vy * 0.90));
        return UDim2.fromOffset(w, h);
    end;

    local isTablet = (math.min(vx, vy) >= 600) or (vx >= 1000 and vy >= 650);
    if isTablet then
        local w = math.clamp(math.floor(vx * 0.74), 680, 820);
        local h = math.clamp(math.floor(vy * 0.72), 480, 580);
        return UDim2.fromOffset(w, h);
    end;

    -- Mobile phone (landscape orientation):
    -- Height: fills ~88% of vertical space, leaving safe margins top/bottom
    local targetHeight = math.floor(vy * 0.88);
    local h = math.clamp(targetHeight, 280, math.max(vy - 28, 280));

    -- Width: proportional card layout (~80% screen width, capped by 2.1x aspect ratio and screen width)
    local targetWidth = math.floor(vx * 0.80);
    local maxWidth = math.min(math.floor(h * 2.1), vx - 32);
    local minWidth = math.min(500, math.max(vx - 24, 300));
    local w = math.clamp(targetWidth, minWidth, maxWidth);

    return UDim2.fromOffset(w, h);
end;

function Library:GetMobileWindowSize()
    return self:GetResponsiveWindowSize();
end;
...
do
    ...
    function Funcs:AddKeyPicker(Idx, Info)
        ...
        Library:AddStroke(PickInner, 'OutlineColor', 1.2);
            ...
            if Library:IsPointerInput(Input) and not Library:MouseIsOverOpenedFrame() then
                ...
                Event = InputService.InputBegan:Connect(function(Input)
                    ...
                    if Input.UserInputType == Enum.UserInputType.Keyboard then
                        if Input.KeyCode == Enum.KeyCode.Escape or Input.KeyCode.Name == 'Escape' then
                            Key = 'None';
                        else
...
do
    ...
    function Funcs:AddButton(...)
        ...
        local function CreateBaseButton(Button)
            ...
            Outer.MouseEnter:Connect(function()
                ...
                Label.TextColor3 = Library.FontColor;
                Outer.BackgroundColor3 = Library.BackgroundColor:Lerp(Library.AccentColor, 0.22);
            end);
            ...
            Outer.InputBegan:Connect(function(Input)
                if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
                    Outer.BackgroundColor3 = Library.BackgroundColor:Lerp(Library.AccentColor, 0.34);
                end;
            end);
    ...
    function Funcs:AddInput(Idx, Info)
        ...
        Library:AddStroke(TextBoxInner, 'OutlineColor', 1.2);

        Library:AddToRegistry(TextBoxInner, {
            BackgroundColor3 = 'BackgroundColor';
        });

        local TextBoxStroke = TextBoxInner:FindFirstChild('YunoStroke');
        ...
        TextBoxOuter.MouseLeave:Connect(function()
            if TextBoxStroke then
                TextBoxStroke.Color = Library.OutlineColor;
            end;
        end);
    ...
    function Funcs:AddToggle(Idx, Info)
        ...
        local ToggleStroke = Library:AddStroke(ToggleInner, 'OutlineColor', 1.2);
        ...
        ToggleRegion.MouseLeave:Connect(function()
            if not Toggle.Value then
                ToggleStroke.Color = Library.OutlineColor;
            end;
        end);
        ...
        function Toggle:Display()
            ...
            ToggleStroke.Thickness = Toggle.Value and 1.4 or 1.2;

            Library.RegistryMap[ToggleInner].Properties.BackgroundColor3 = Toggle.Value and 'AccentColor' or 'BackgroundColor';
            local strokeReg = Library.RegistryMap[ToggleStroke];
            if strokeReg and strokeReg.Properties then
                strokeReg.Properties.Color = Toggle.Value and 'AccentColor' or 'OutlineColor';
            end;
        end;
    ...
    function Funcs:AddSlider(Idx, Info)
        ...
        local SliderOuter = Library:Create('Frame', {
            ...
            BackgroundTransparency = 1;
            BorderSizePixel = 0;
            Size = UDim2.new(1, -4, 0, Library:IsMobile() and 18 or 14);
            ZIndex = 5;
            Parent = Container;
        });
        ...
        Library:AddStroke(SliderInner, 'OutlineColor', 1.2);
        ...
        local SliderStroke = SliderInner:FindFirstChild('YunoStroke');
        SliderOuter.MouseEnter:Connect(function()
            if SliderStroke then
                SliderStroke.Color = Library.AccentColor;
            end;
        end);
        SliderOuter.MouseLeave:Connect(function()
            if SliderStroke then
                SliderStroke.Color = Library.OutlineColor;
            end;
        end);
    ...
    function Funcs:AddDropdown(Idx, Info)
        ...
        Library:AddStroke(DropdownInner, 'OutlineColor', 1.2);
        ...
        local DropdownStroke = DropdownInner:FindFirstChild('YunoStroke');
...
function Library:CreateWindow(...)
    ...
    if typeof(Config.Size) ~= 'UDim2' then Config.Size = UDim2.fromOffset(840, 600) end

    if Library:IsMobile() then
        Config.Size = Library:GetMobileWindowSize();
        Config.Center = true;
    elseif typeof(Config.Size) == 'UDim2' then
        local vp = Library:GetViewportSize();
        if Config.Size.X.Offset > (vp.X - 30) or Config.Size.Y.Offset > (vp.Y - 30) then
            Config.Size = Library:GetResponsiveWindowSize();
            Config.Center = true;
        end;
    end;
    ...
    if camera then
        ...
        Library:GiveSignal(camera:GetPropertyChangedSignal('ViewportSize'):Connect(function()
            local vp = Library:GetViewportSize();
            if lastVp and lastVp.X == vp.X and lastVp.Y == vp.Y then
                return;
            end;
            lastVp = vp;

            if Library.Toggled ~= true and (not Outer or not Outer.Visible) then
                return;
            end;
            ...
            if Library:IsMobile() then
                ...
                if Outer.AbsoluteSize.X > (vp.X - 20) or Outer.AbsoluteSize.Y > (vp.Y - 20) then
                    Outer.Size = Library:GetResponsiveWindowSize();
                    Library:ClampGuiToViewport(Outer);
                end;
            end;
        end));
    ...
    pcall(function()
        WindowLabel.Interactable = false;
    end);
    ...
    Library:AddToRegistry(TitleRule, {
    ...
    local function RelayoutWindowTabs()
        ...
        task.defer(function()
            ...
            local textSize = (count >= 11) and 12 or 13;
    ...
    Library:AddCorner(TabContainer, 2);
    ...
    function Window:AddTab(Name)
        ...
        local TabButton = Library:Create('Frame', {
            ...
            BackgroundTransparency = 1;
            BorderSizePixel = 0;
            Size = UDim2.fromOffset(64, TAB_BUTTON_HEIGHT);
        ...
        function Tab:ShowTab()
            ...
            TabButton.BackgroundColor3 = Library.MainColor:Lerp(Library.AccentColor, 0.32);
            TabButtonLabel.TextColor3 = Library.FontColor;
            TabButtonLabel.TextStrokeTransparency = 0.4;
        ...
        function Tab:HideTab()
            ...
            TabButtonLabel.TextColor3 = Library.FontColor:Lerp(Library.OutlineColor, 0.12);
            TabButtonLabel.TextStrokeTransparency = 0.45;
            local registry = Library.RegistryMap[TabButton];
            if registry and registry.Properties then
                registry.Properties.BackgroundColor3 = 'BackgroundColor';
            end;
        ...
        function Tab:AddTabbox(Info)
            ...
            function Tabbox:AddTab(Name)
                ...
                local Button = Library:Create('Frame', {
                    Active = true;
                    BackgroundColor3 = Color3.fromRGB(14, 11, 22);
                    BorderColor3 = Color3.fromRGB(30, 24, 46);
                    BorderSizePixel = 1;
                ...
                Button.MouseEnter:Connect(function()
                    if not Container.Visible then
                        Library:Tween(ButtonLabel, 0.15, { TextColor3 = Color3.fromRGB(220, 220, 230) });
                        Library:Tween(Button, 0.15, { BackgroundColor3 = Color3.fromRGB(22, 18, 34) });
                    end;
                end);

                Button.MouseLeave:Connect(function()
                    if not Container.Visible then
                        Library:Tween(ButtonLabel, 0.15, { TextColor3 = Color3.fromRGB(150, 150, 165) });
                        Library:Tween(Button, 0.15, { BackgroundColor3 = Color3.fromRGB(14, 11, 22) });
                    end;
                end);
                ...
                function Tab:Hide()
                    ...
                    Button.BackgroundColor3 = Color3.fromRGB(14, 11, 22);
                    ButtonLabel.TextColor3 = Color3.fromRGB(150, 150, 165);
                end;

                function Tab:Resize()
                    if Library:IsLayoutSuspended() then
                        Library.PendingResizes[Tab] = true;
                        return;
                    end;
                    ...
                    local pad = ContainerLayout.Padding.Offset;
                    if visibleCount > 1 then
                        Size = Size + (visibleCount - 1) * pad;
                    end;

                    BoxOuter.Size = UDim2.new(1, 0, 0, 24 + Size + 12);
                end;
        ...
        TabButton.MouseEnter:Connect(function()
            if not TabFrame.Visible then
                TabButton.BackgroundTransparency = 0;
                TabButton.BackgroundColor3 = Library.BackgroundColor:Lerp(Library.AccentColor, 0.18);
                TabButtonLabel.TextColor3 = Library.FontColor;
            end;
        end);
        ...
        if Toggled then
            ...
            if pending and type(pending.ShowTab) == 'function' then
                ...
                for _, tab in next, Window.Tabs do
                    if tab.TabFrame and tab.TabFrame.Visible then
                        anyVisible = true;
                        break;
                    end;
                end;
                if not anyVisible then
                    local firstTab = Window.Tabs[1] or (Window.TabButtons[1] and Window.TabButtons[1].Tab);
            ...
            Outer.Visible = true;
            if type(RelayoutWindowTabs) == 'function' then
                pcall(RelayoutWindowTabs);
            end;
            Library:EnsureMenuCursor();
        else
            Outer.Visible = false;
